...
These libraries are included in the EaselJS repository, which makes it easier to run the build, and ensures that we don't have version mismatches between the source and the libraries.
...
<pre><code>Usage
node ./build.js [-v] [-h] [-l] --tasks=TASK [--version=DOC_VERSION]
...
Options:
  ...
  --tasks        Task to run                    [default: "all"]</code></pre>
...
<code>./build.js --tasks=BUILDDOCS</code>
