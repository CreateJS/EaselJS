...
function(){this.beginStroke();return this};a.curveTo=a.quadraticCurveTo;a.drawRect=a.rect;a.drawRoundRect=function(a,b,d,c,f){this.drawRoundRectComplex(a,b,d,c,f,f,f,f);return this};a.drawRoundRectComplex=function(a,b,d,e,f,g,i,j){var l=(d<e?d:e)/2,m=0,o=0,n=0,q=0;f<0&&(f*=m=-1);f>l&&(f=l);g<0&&(g*=o=-1);g>l&&(g=l);i<0&&(i*=n=-1);i>l&&(i=l);j<0&&(j*=q=-1);j>l&&(j=l);this._dirty=this._active=true;var l=this._ctx.arcTo,p=this._ctx.lineTo;this._activeInstructions.push(new c(this._ctx.moveTo,[a+d-g,b]),
new c(l,[a+d+g*o,b-g*o,a+d,b+g,g]),new c(p,[a+d,b+e-i]),new c(l,[a+d+i*n,b+e+i*n,a+d-i,b+e,i]),new c(p,[a+j,b+e]),new c(l,[a-j*q,b+e+j*q,a,b+e-j,j]),new c(p,[a,b+f]),new c(l,[a-f*m,b-f*m,a+f,b,f]),new c(this._ctx.closePath));return this};a.drawCircle=function(a,b,d){this.arc(a,b,d,0,Math.PI*2);return this};a.drawEllipse=function(a,b,d,e){this._dirty=this._active=true;var f=d/2*0.5522848,g=e/2*0.5522848,i=a+d,j=b+e,d=a+d/2,e=b+e/2;this._activeInstructions.push(new c(this._ctx.moveTo,[a,e]),new c(this._ctx.bezierCurveTo,
...
0,f.width=1,this;for(var j=this.children.length-1;j>=0;j--){var l=this.children[j];if(l.isVisible()&&l.mouseEnabled)if(l instanceof c)if(i){if(l=l._getObjectsUnderPoint(a,b))return this}else{if(l=l._getObjectsUnderPoint(a,b,h,d),!h&&l)return l}else if(!d||i||l._hasMouseHandler(d)){var m=l.hitArea;l.getConcatenatedMatrix(g);m&&(g.appendTransform(m.x+l.regX,m.y+l.regY,m.scaleX,m.scaleY,m.rotation,m.skewX,m.skewY,m.regX,m.regY),g.alpha*=m.alpha/l.alpha);e.globalAlpha=g.alpha;e.setTransform(g.a,g.b,g.c,
g.d,g.tx-a,g.ty-b);(m||l).draw(e);if(this._testHit(e))if(f.width=0,f.width=1,i)return this;else if(h)h.push(l);else return l}}return null};createjs.Container=c})();this.createjs=this.createjs||{};
...
function(b,a,k,h){for(var d=b._images,e=c._workingCanvas,f=c._workingContext,g=d.length/a,i=0;i<g;i++){var j=d[i];j.__tmp=i;e.width=0;e.width=j.width;e.height=j.height;f.setTransform(k?-1:1,0,0,h?-1:1,k?j.width:0,h?j.height:0);f.drawImage(j,0,0);var l=new Image;l.src=e.toDataURL("image/png");l.width=j.width;l.height=j.height;d.push(l)}f=b._frames;e=f.length/a;for(i=0;i<e;i++){var j=f[i],m=j.rect.clone(),l=d[j.image.__tmp+g*a],o={image:l,rect:m,regX:j.regX,regY:j.regY};if(k)m.x=l.width-m.x-m.width,
o.regX=m.width-j.regX;if(h)m.y=l.height-m.y-m.height,o.regY=m.height-j.regY;f.push(o)}k="_"+(k?"h":"")+(h?"v":"");h=b._animations;b=b._data;d=h.length/a;for(i=0;i<d;i++){f=h[i];j=b[f];g={name:f+k,frequency:j.frequency,next:j.next,frames:[]};j.next&&(g.next+=k);f=j.frames;j=0;for(l=f.length;j<l;j++)g.frames.push(f[j]+e*a);b[g.name]=g;h.push(g.name)}};createjs.SpriteSheetUtils=c})();this.createjs=this.createjs||{};
...
i.width=this._getSize(e,this.maxWidth);i.height=this._getSize(d,this.maxHeight);this._data.images[f]=i;g.h||(e=d=0,f++)}}};b._getSize=function(a,b){for(var c=4;Math.pow(2,++c)<a;);return Math.min(b,Math.pow(2,c))};b._fillRow=function(a,b,h,d,e){var f=this.maxWidth,g=this.maxHeight-b,i=e;b+=e;for(var j=0,l=a.length-1;l>=0;l--){var m=a[l],o=this._scale*m.scale,n=m.sourceRect,q=m.source,p=Math.floor(o*n.x-e),s=Math.floor(o*n.y-e),r=Math.ceil(o*n.height+e*2),n=Math.ceil(o*n.width+e*2);if(n>f)throw c.ERR_DIMENSIONS;
if(!(r>g||i+n>f))m.img=h,m.rect=new createjs.Rectangle(i,b,n,r),j=j||r,a.splice(l,1),d[m.index]=[i,b,n,r,h,Math.round(-p+o*q.regX-e),Math.round(-s+o*q.regY-e)],i+=n}return{w:i,h:j}};b._endBuild=function(){this.spriteSheet=new createjs.SpriteSheet(this._data);this._data=null;this._callback&&this._callback(this)};b._run=function(){for(var a=(new Date).getTime()+this._timeSlice,b=false;a>(new Date).getTime();)if(!this._drawNext()){b=true;break}if(b)this._endBuild();else{var c=this;this._timerID=setTimeout(function(){c._run()},
