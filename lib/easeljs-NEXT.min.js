...
e.y,c);return c};a.getObjectUnderPoint=function(b,a){var c=this.localToGlobal(b,a);return this._getObjectsUnderPoint(c.x,c.y)};a.clone=function(b){var a=new c;this.cloneProps(a);if(b)for(var i=a.children=[],e=0,d=this.children.length;e<d;e++){var f=this.children[e].clone(b);f.parent=a;i.push(f)}return a};a.toString=function(){return"[Container (name="+this.name+")]"};a._tick=function(b){for(var a=this.children.length-1;a>=0;a--){var c=this.children[a];c._tick&&c._tick(b)}if(this.onTick)this.onTick(b)};
a._getObjectsUnderPoint=function(b,a,i,e){var d=DisplayObject._hitTestContext,f=DisplayObject._hitTestCanvas,h=this._matrix,l=e&1&&(this.onPress||this.onClick||this.onDoubleClick)||e&2&&(this.onMouseOver||this.onMouseOut);if(this.cacheCanvas&&l&&(this.getConcatenatedMatrix(h),d.setTransform(h.a,h.b,h.c,h.d,h.tx-b,h.ty-a),d.globalAlpha=h.alpha,this.draw(d),this._testHit(d)))return f.width=0,f.width=1,this;for(var k=this.children.length-1;k>=0;k--){var j=this.children[k];if(j.isVisible()&&j.mouseEnabled)if(j instanceof
c)if(l){if(j=j._getObjectsUnderPoint(b,a))return this}else{if(j=j._getObjectsUnderPoint(b,a,i,e),!i&&j)return j}else if(!e||l||e&1&&(j.onPress||j.onClick||j.onDoubleClick)||e&2&&(j.onMouseOver||j.onMouseOut)){var g=j.hitArea;j.getConcatenatedMatrix(h);g&&(h.appendTransform(g.x+j.regX,g.y+j.regY,g.scaleX,g.scaleY,g.rotation,g.skewX,g.skewY,g.regX,g.regY),h.alpha*=g.alpha/j.alpha);d.globalAlpha=h.alpha;d.setTransform(h.a,h.b,h.c,h.d,h.tx-b,h.ty-a);(g||j).draw(d);if(this._testHit(d))if(f.width=0,f.width=
1,l)return this;else if(i)i.push(j);else return j}}return null};g.Container=c})(window);(function(g){var c=function(b){this.initialize(b)},a=c.prototype=new Container;c._snapToPixelEnabled=false;a.autoClear=true;a.canvas=null;a.mouseX=null;a.mouseY=null;a.onMouseMove=null;a.onMouseUp=null;a.onMouseDown=null;a.snapToPixelEnabled=false;a.mouseInBounds=false;a.tickOnUpdate=true;a._activeMouseEvent=null;a._activeMouseTarget=null;a._mouseOverIntervalID=null;a._mouseOverX=0;a._mouseOverY=0;a._mouseOverTarget=null;a.Container_initialize=a.initialize;a.initialize=function(b){this.Container_initialize();
