...
0,f.width=1,this;for(var j=this.children.length-1;j>=0;j--){var l=this.children[j];if(l.isVisible()&&l.mouseEnabled){var m=l.hitArea,n;if(l instanceof c&&(!m||!(n=l._hasMouseHandler(d))))if(i){if(l=l._getObjectsUnderPoint(a,b))return this}else{if(l=l._getObjectsUnderPoint(a,b,h,d),!h&&l)return l}else if(!d||i||n||l._hasMouseHandler(d))if(l.getConcatenatedMatrix(g),m&&(g.appendTransform(m.x+l.regX,m.y+l.regY,m.scaleX,m.scaleY,m.rotation,m.skewX,m.skewY,m.regX,m.regY),g.alpha*=m.alpha/l.alpha),e.globalAlpha=
g.alpha,e.setTransform(g.a,g.b,g.c,g.d,g.tx-a,g.ty-b),(m||l).draw(e),this._testHit(e))if(f.width=0,f.width=1,i)return this;else if(h)h.push(l);else return l}}return null};createjs.Container=c})();this.createjs=this.createjs||{};
