...
b.shadow,b.compositeOperation);return this};a.prependTransform=function(b,a,h,e,d,f,g,l,k){if(d%360)var j=d*c.DEG_TO_RAD,d=Math.cos(j),j=Math.sin(j);else d=1,j=0;if(l||k)this.tx-=l,this.ty-=k;f||g?(f*=c.DEG_TO_RAD,g*=c.DEG_TO_RAD,this.prepend(d*h,j*h,-j*e,d*e,0,0),this.prepend(Math.cos(g),Math.sin(g),-Math.sin(f),Math.cos(f),b,a)):this.prepend(d*h,j*h,-j*e,d*e,b,a);return this};a.appendTransform=function(b,a,h,e,d,f,g,l,k){if(d%360)var j=d*c.DEG_TO_RAD,d=Math.cos(j),j=Math.sin(j);else d=1,j=0;f||
...
[];this._data={};for(var g in c){b={name:g};f=c[g];if(isNaN(f))if(f instanceof Array){b.frequency=f[3];b.next=f[2];e=b.frames=[];for(a=f[0];a<=f[1];a++)e.push(a)}else b.frequency=f.frequency,b.next=f.next,a=f.frames,e=b.frames=!isNaN(a)?[a]:a.slice(0);else e=b.frames=[f];b.next=e.length<2||b.next==false?null:b.next==null||b.next==true?g:b.next;if(!b.frequency)b.frequency=1;this._animations.push(g);this._data[g]=b}}}};a.getNumFrames=function(b){return b==null?this._frames?this._frames.length:this._numFrames:
...
a+Math.sin(g)*e]));return this};b.decodePath=function(b){for(var c=[this.moveTo,this.lineTo,this.quadraticCurveTo,this.bezierCurveTo],e=[2,2,4,6],d=0,f=b.length,g=[],l=0,k=0,j=a.BASE_64;d<f;){var i=j[b.charAt(d)],n=i>>3,r=c[n];if(!r||i&3)throw"bad path data";var s=e[n];n||(l=k=0);g.length=0;d++;i=(i>>2&1)+2;for(n=0;n<s;n++){var o=j[b.charAt(d)],q=o>>5?-1:1,o=(o&31)<<6|j[b.charAt(d+1)];i==3&&(o=o<<6|j[b.charAt(d+2)]);o=q*o/10;n%2?l=o+=l:k=o+=k;g[n]=o;d+=i}r.apply(this,g)}return this};b.clone=function(){var b=
...
this.outline;b.lineHeight=this.lineHeight;b.lineWidth=this.lineWidth};a._getWorkingContext=function(){var b=c._workingContext;b.font=this.font;b.textAlign=this.textAlign||"start";b.textBaseline=this.textBaseline||"alphabetic";return b};a._drawText=function(b){var a=!!b;a||(b=this._getWorkingContext());for(var c=String(this.text).split(/(?:\r\n|\r|\n)/),e=this.lineHeight||this.getMeasuredLineHeight(),d=0,f=0,g=c.length;f<g;f++){var l=b.measureText(c[f]).width;if(this.lineWidth==null||l<this.lineWidth)a&&
this._drawTextLine(b,c[f],d*e);else{for(var l=c[f].split(/(\s)/),k=l[0],j=1,i=l.length;j<i;j+=2)b.measureText(k+l[j]+l[j+1]).width>this.lineWidth?(a&&this._drawTextLine(b,k,d*e),d++,k=l[j+1]):k+=l[j]+l[j+1];a&&this._drawTextLine(b,k,d*e)}d++}return d};a._drawTextLine=function(b,a,c){this.outline?b.strokeText(a,0,c,this.maxWidth||65535):b.fillText(a,0,c,this.maxWidth||65535)};i.Text=c})(window);(function(i){var c=function(){throw"SpriteSheetUtils cannot be instantiated";};c._workingCanvas=document.createElement("canvas");c._workingContext=c._workingCanvas.getContext("2d");c.addFlippedFrames=function(a,b,m,h){if(b||m||h){var e=0;b&&c._flip(a,++e,true,false);m&&c._flip(a,++e,false,true);h&&c._flip(a,++e,true,true)}};c.extractFrame=function(a,b){isNaN(b)&&(b=a.getAnimation(b).frames[0]);var m=a.getFrame(b);if(!m)return null;var h=m.rect,e=c._workingCanvas;e.width=h.width;e.height=h.height;
c._workingContext.drawImage(m.image,h.x,h.y,h.width,h.height,0,0,h.width,h.height);m=new Image;m.src=e.toDataURL("image/png");return m};c._flip=function(a,b,m,h){for(var e=a._images,d=c._workingCanvas,f=c._workingContext,g=e.length/b,l=0;l<g;l++){var k=e[l];k.__tmp=l;d.width=k.width;d.height=k.height;f.setTransform(m?-1:1,0,0,h?-1:1,m?k.width:0,h?k.height:0);f.drawImage(k,0,0);var j=new Image;j.src=d.toDataURL("image/png");j.width=k.width;j.height=k.height;e.push(j)}f=a._frames;d=f.length/b;for(l=
0;l<d;l++){var k=f[l],i=k.rect.clone(),j=e[k.image.__tmp+g*b],n={image:j,rect:i,regX:k.regX,regY:k.regY};if(m)i.x=j.width-i.x-i.width,n.regX=i.width-k.regX;if(h)i.y=j.height-i.y-i.height,n.regY=i.height-k.regY;f.push(n)}m="_"+(m?"h":"")+(h?"v":"");h=a._animations;a=a._data;e=h.length/b;for(l=0;l<e;l++){f=h[l];k=a[f];g={name:f+m,frequency:k.frequency,next:k.next,frames:[]};k.next&&(g.next+=m);f=k.frames;k=0;for(j=f.length;k<j;k++)g.frames.push(f[k]+d*b);a[g.name]=g;h.push(g.name)}};i.SpriteSheetUtils=
c})(window);(function(i){var c=function(){this.initialize()},a=c.prototype;c.ERR_DIMENSIONS="frame dimensions exceed max spritesheet dimensions";c.ERR_RUNNING="a build is already running";a.maxWidth=2048;a.maxHeight=2048;a.spriteSheet=null;a.scale=1;a._frames=null;a._animations=null;a._data=null;a._nextFrameIndex=0;a._index=0;a._callback=null;a._timeSlice=null;a._timerID=null;a._scale=1;a.initialize=function(){this._frames=[];this._animations={}};a.addFrame=function(a,m,h,e,d,f){if(this._data)throw c.ERR_RUNNING;
m=m||a.bounds||a.nominalBounds;!m&&a.getBounds&&(m=a.getBounds());if(!m)return null;h=h||1;return this._frames.push({source:a,sourceRect:m,scale:h,funct:e,params:d,scope:f,index:this._frames.length,height:m.height*h})-1};a.addAnimation=function(a,m,h,e){if(this._data)throw c.ERR_RUNNING;this._animations[a]={frames:m,next:h,frequency:e}};a.addMovieClip=function(a,m,h){if(this._data)throw c.ERR_RUNNING;var e=a.frameBounds,m=m||a.bounds||a.nominalBounds;!m&&a.getBounds&&(m=a.getBounds());if(!m&&!e)return null;
for(var d=a.timeline.duration,f=0;f<d;f++)this.addFrame(a,e&&e[f]?e[f]:m,h,function(a){var b=this.actionsEnabled;this.actionsEnabled=false;this.gotoAndStop(a);this.actionsEnabled=b},[f],a);var a=a.timeline._labels,g;for(g in a)this.addAnimation(g,a[g],true)};a.build=function(){if(this._data)throw c.ERR_RUNNING;this._callback=null;for(this._startBuild();this._drawNext(););this._endBuild();return this.spriteSheet};a.buildAsync=function(a,m){if(this._data)throw c.ERR_RUNNING;this._callback=a;this._startBuild();
this._timeSlice=Math.max(0.01,Math.min(0.99,m||0.3))*50;var h=this;this._timerID=setTimeout(function(){h._run()},50-this._timeSlice)};a.stopAsync=function(){clearTimeout(this._timerID);this._data=null};a.clone=function(){throw"SpriteSheet cannot be cloned.";};a.toString=function(){return"[SpriteSheetBuilder]"};a._startBuild=function(){this.spriteSheet=null;this._index=0;this._scale=this.scale;var a=[];this._data={images:[],frames:a,animations:this._animations};var m=this._frames.slice();m.sort(function(a,
b){return a.height<=b.height?-1:1});if(m[m.length-1].height>this.maxHeight)throw c.ERR_DIMENSIONS;for(var h=1,e=1,d=0;m.length;){var f=this._fillRow(m,h,d,a);if(f.w>e)e=f.w;h+=f.h+2;if(!f.h||!m.length){var g=document.createElement("canvas");g.width=this._getSize(e,this.maxWidth);g.height=this._getSize(h,this.maxHeight);this._data.images[d]=g;f.h||(e=h=1,d++)}}};a._getSize=function(a,c){for(var h=4;Math.pow(2,++h)<a;);return Math.min(c,Math.pow(2,h))};a._fillRow=function(a,m,h,e){for(var d=this.maxWidth,
f=this.maxHeight-m,g=1,i=0,k=a.length-1;k>=0;k--){var j=a[k],p=this._scale*j.scale,n=j.sourceRect,r=j.source,s=Math.floor(p*n.x-1),o=Math.floor(p*n.y-1),q=Math.ceil(p*n.height),n=Math.ceil(p*n.width);if(n>d)throw c.ERR_DIMENSIONS;if(!(q>f||g+n>d))j.img=h,j.rect=new Rectangle(g,m,n,q),i=i||q,a.splice(k,1),e[j.index]=[g-1,m-1,n+2,q+2,h,Math.round(-s+p*r.regX),Math.round(-o+p*r.regY)],g+=n+2}return{w:g,h:i}};a._endBuild=function(){this.spriteSheet=new SpriteSheet(this._data);this._data=null;this._callback&&
this._callback(this)};a._run=function(){for(var a=(new Date).getTime()+this._timeSlice,c=false;a>(new Date).getTime();)if(!this._drawNext()){c=true;break}if(c)this._endBuild();else{var h=this;this._timerID=setTimeout(function(){h._run()},50-this._timeSlice)}};a._drawNext=function(){var a=this._frames[this._index],c=a.scale*this._scale,h=a.rect,e=a.sourceRect,d=this._data.images[a.img].getContext("2d");a.funct&&a.funct.apply(a.scope,a.params);d.save();d.beginPath();d.rect(h.x-1,h.y-1,h.width+2,h.height+
